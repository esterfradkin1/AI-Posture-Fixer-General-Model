...
def predict_from_features_dict(features_dict):
    # יצירת DataFrame - פותר את האזהרות ומבטיח סדר עמודות
    ...
    # המרה חזרה למילה (Bad/Good Posture)
    # שימוש ב-LabelEncoder שנטען מה-pack
    label = pack["label_encoder"].inverse_transform([label_encoded])[0]

    prob_bad = None
    if hasattr(model, "predict_proba"):
        probs = model.predict_proba(x_scaled)[0]
        # ניסיון חכם למצוא את האינדקס של Bad
        ...
        try:
            ...
            # אם לא נמצא, ננסה לפי אינדקס 0 (בדרך כלל Bad הוא הראשון)
