...
# =========================
PACK_PATH = r"C:\Users\ester\Desktop\Ai posture fixer - code project\video_to_frames\pythonProject1\models\posture_knn_blackbox_new.joblib"
...
BAD_ON_RATIO = 0.5         # Good → Bad threshold
...
def predict_from_features_dict(features_dict):
    # Create DataFrame - resolves warnings and guarantees column order
    x_df = pd.DataFrame([features_dict])[feature_names]

    if too_many_missing(x_df.values):
        return "Unknown", None
    ...
    # Prediction
    label_encoded = model.predict(x_scaled)[0]

    # Convert back to label string (Bad/Good Posture)
    # Using LabelEncoder loaded from the pack
    label = pack["label_encoder"].inverse_transform([label_encoded])[0]

    prob_bad = None
    if hasattr(model, "predict_proba"):
        probs = model.predict_proba(x_scaled)[0]
        # Intelligently find the index for Bad
        cls_names = [str(c).lower() for c in pack["label_encoder"].classes_]
        try:
            bad_idx = cls_names.index("bad_posture")
            prob_bad = float(probs[bad_idx])
        except ValueError:
            # If not found, try index 0 (typically Bad is the first class)
